    ...
    public function store(Requests\ContatoRequest $request)
    {
        ...
        if(isset($_POST['g-recaptcha-response'])&&($_POST['g-recaptcha-response'])){
            ...
            if ($arr['success']){
                ...
                $data =  $request->only('Nome', 'Telefone', 'Email','Assunto','Mensagem');

               Mail::send('emails.contato',$data, function($mensagem) use($data){
                    $mensagem->to('diegorodsouza1997@gmail.com', 'DOGE')
                             ->from('suacasasuavida@imob.com.br', 'Imobiliária')
